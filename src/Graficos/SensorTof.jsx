...
const SensorTof = () => {
  ...
  // const sensorTofDist = (Math.random() * 100)
  const sensorTofDist = 78
  ...
  useEffect(() => {
    if (sensorData.length > 0) { 
      ...
      const tempCtx = document.getElementById('GraphSensorTof');
      ...
      const newTempChartInstance = new Chart(tempCtx, {
        ...
        data: {
          ...
          datasets: [
            {
              data: [sensorTofDist, 100-sensorTofDist],
              borderRadius: [20, 0],
              borderWidth: 2,
              borderColor: 'transparent',
              backgroundColor: ['#eeba57', 'rgb(0,0,0,0)'],
  ...
  return (
    <div className='Body'>
      <div className='BodyLegendas'>
        <p className='TextLegendas'>Sensor TOF</p>
        <p className='TextLegendasNum'> { sensorTofDist.toFixed(3) } MM </p>
        <p className='TextLegendaSensorTof'>Distancia em milimetro</p>
      </div>
      <div className='BodyGrafico'>
        <canvas id="GraphSensorTof" width="50" height="50"></canvas>
      </div>
    </div>
  );
};

export default SensorTof;
